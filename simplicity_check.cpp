...
long long gcd(long long a, long long b) { // Нахождение НОДа алгоритмом Евклида
    if (b == 0)
        return a;
    return gcd(b, a % b);
}
long long mul(long long a, long long b, long long m) { // функция двоичного умножения по модулю (чтобы не было переполнения типа "long long" еще до того, как будет взят результат по модулю)
    if (b == 1)
        return a;
    if (b % 2 == 0) {
...
bool MillerRabinTest(long n, int k) {   // производится k раундов проверки числа n на простоту тестом Миллера-Рабина
...
bool SoloveiShtrassenTest(long n, int k) {   // производится k раундов проверки числа n на простоту тестом Соловея-Штрассена
    auto b = 0;
    auto c = 0;
    // если n == 2 или n == 3 - эти числа простые, возвращаем true
    if (n == 2 || n == 3)
    ...
    for (int i = 0; i < k; i++)
    {
        // выберем случайное целое число a в отрезке [2, n − 1]
        random_device rd;
        ...
        if (gcd(n, a) > 1) { // Если НОД > 1, то число составное
            return false;
        }
    ...
    // вернуть "n - простое с вероятностью 1 - 2^-k"
    return true;
}
